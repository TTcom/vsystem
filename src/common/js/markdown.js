...
;(function(expose) {
  ...
  var Markdown = function(dialect) {
    switch (typeof dialect) {
      ...
      case "object":
        ...
        break
      default:
        if (dialect in Markdown.dialects) this.dialect = Markdown.dialects[dialect]
  ...
  Markdown.renderJsonML = function(jsonml, options) {
    ...
    if (options.root) {
      ...
      if (jsonml.length && typeof jsonml[0] === "object" && !(jsonml[0] instanceof Array)) jsonml.shift() // get rid of the attributes
      ...
      lists: (function() {
        ...
        function add(li, loose, inline, nl) {
          ...
          for (var i = 0; i < inline.length; i++) {
            ...
            if (is_str && add_to.length > 1 && typeof add_to[add_to.length - 1] === "string") add_to[add_to.length - 1] += what
    ...
    inline: {
      ...
      "<": function autoLink(text) {
        ...
        if ((m = text.match(/^<(?:((https?|ftp|mailto):[^>]+)|(.*?@.*?\.[a-zA-Z]+))>/)) !== null) {
          if (m[3]) return [m[0].length, ["link", { href: "mailto:" + m[3] }, m[3]]]
  ...
  Maruku.processMetaHash = function processMetaHash(meta_string) {
    ...
    for (var i = 0; i < meta.length; ++i) {
      ...
      // class: .foo
        ...
        if (attr["class"]) attr["class"] = attr["class"] + meta[i].replace(/./, " ")
